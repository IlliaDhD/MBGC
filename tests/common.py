...
class TestTransactions:
    ...
    def generate_test_data(
        ...
        for _ in range(cls.rand_num(b=30)):  # the loop adds existing institutions
            institution = random.choice(institutions)
            data_rows.append({'description': institution})
            # creates an expected result
            result_rows.append({'description': institution, 'operationCategory': categories[mappings[institution]]})
...
class TestPeriods:
    ...
    @staticmethod
    def generate_period(start_time: float) -> Tuple[int, List[Tuple[float, float]]]:
