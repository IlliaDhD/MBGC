...
class TestTimeBounds(unittest.TestCase):
    ...
    def test_bounds_from_period(self):
        ...
        with patch("app.api.time_bounds.time.time",
                   return_value=self.mocked_time):
    ...
    def test_bounds_from_period_less_than_a_month(self):
        period, expected_bounds = TestPeriods.generate_period_with_days_number(
            self.mocked_time, random.randint(1, 30))
